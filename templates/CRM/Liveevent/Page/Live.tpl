...
@media print {
  * {font-family:Helvetica, Arial, sans serif}
  ...
  h1 {text-shadow:none;}
  #logo {display:none;}
  #wrapper {position:inherit;display:none!important;visibility:hidden;}
...
/*  .fbadge,.vbadge {width:97mm!important;height:86mm!important;position:fixed;bottom:0;left:0;visibility:visible!important;display:block;}*/
  .fbadge,.vbadge {width:50%!important;height:86mm!important;position:fixed;bottom:0mm;left:0;visibility:visible!important;display:block;}
  .fbadge img,.vbadge img {width:50%;height:86mm;z-index:2} 
...
{/literal}{$cj}{literal} (function($){
  ...
  var setParticipant =function (data) {
    ...
    $('#badge').addClass('status_'+data.status_id);
...
/*    var data= "BEGIN:VCARD\nVERSION:3.0\nFN:"+contact.first_name+" "+contact.last_name;
    ...
    var data="MECARD:N:"+contact.last_name+","+contact.first_name+';NOTE:'+contact.id+';';
    if (contact.organization_name)
...
//    jQuery(selector).html('').qrcode({text:data,width:78,height:78,typeNumber: 8,correctLevel: 0});
  ...
  select: function( event, ui ) {
    ...
    $('#badge').addClass('status_'+ui.item.status_id);
...
function setBackground (role_id) {
  var img= {3:'staff.jpg',8:'tech.jpg',7:'press.jpg',11:'press.jpg',10:'security.jpg',4:'vip.jpg',12:'security_vip.jpg'};
  var src = {/literal}'/{$tttp_root}/images/Badge/participant.jpg'{literal};
  if (typeof img[role_id] != "undefined")
    src = {/literal}'/{$tttp_root}/images/Badge/'+img[role_id]{literal};
